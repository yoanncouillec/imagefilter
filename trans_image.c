...
image_t *symetrie_verticale(image_t *src)
{
  ...
  for (i = 0; i < src->nx; i++)
      ...
      for (j = 0; j < src->ny; j++)
	{
	  ...
	  //printf("okkk %d\n", NBPIXELS(src));
	  //printf("%d %d\n", i,j);
	  //char a =  src->buff[i*src->ny+j]; // (i,j) [i*ny+j] (colonne) [i+j*nx] (ligne)
	  char a =  src->buff[i + src->nx*j]; // (i,j) [i*ny+j] (colonne) [i+j*nx] (ligne)
	  //printf("%d\n", a);
	  //int u = (src->nx-i-1)*src->ny+j; // (nx-i-1,j) [(nx-i-1)*ny+j] [(nx-i-1)+j*nx]
	  int u = (src->nx-i-1)+src->nx*j; // (nx-i-1,j) [(nx-i-1)*ny+j] [(nx-i-1)+j*nx]
	  //printf ("u =%d\n", u); 
	  copie->buff[u] = a;
	  //copie->buff[copie->nx*copie->ny - i*src->ny - (src->ny - j)] = a;
	  //printf("ok\n", NBPIXELS(src));
	}
    }
  return copie;
